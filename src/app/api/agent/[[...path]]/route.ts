...
function buildUrl(path: string[] | undefined, search: string, base: string): string {
  ...
  return `${base}/api/agent${segment ? `/${segment}` : ''}${search || ''}`;
}
...
/**
 * GET handler - Fetches agent-related data
 * Supports paths like:
...
/**
 * POST handler - Creates agent-related resources
 * Supports paths like:
 * - /api/agent/availability (set availability)
 */
...
/**
 * PATCH handler - Updates agent-related data
 * Supports paths like:
