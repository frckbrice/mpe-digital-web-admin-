...
/**
 * API Route: /api/quote-requests/[[...path]]
 *
 * This is a catch-all proxy route that forwards all HTTP methods (GET, POST, PATCH, DELETE)
 * to the MPE Web app's quote-requests API endpoints. This proxy pattern avoids CORS issues and
 * provides a unified API interface for quote request operations.
 *
 * Purpose:
 * - Proxies requests to /api/quote-requests/* endpoints in the MPE Web app
 * - Handles quote request operations (creating, fetching, updating quote requests)
 ...
 * - All quote request operations are handled by the MPE Web app, ensuring single source of truth
 *
 * Note: Quote requests are typically created by clients and managed by admins/agents.
 * This route handles the full lifecycle of quote requests.
 *
 * Error Handling:
...
/**
 * GET handler - Fetches quote requests
 * Supports paths like:
 * - /api/quote-requests (list quote requests)
 * - /api/quote-requests/{id} (get specific quote request)
 */
...
/**
 * POST handler - Creates new quote requests
 * Supports paths like:
 * - /api/quote-requests (create quote request)
 */
...
/**
 * PATCH handler - Updates quote requests
 * Supports paths like:
 * - /api/quote-requests/{id} (update quote request)
 */
...
/**
 * DELETE handler - Deletes quote requests
 * Supports paths like:
 * - /api/quote-requests/{id} (delete quote request)
 */
