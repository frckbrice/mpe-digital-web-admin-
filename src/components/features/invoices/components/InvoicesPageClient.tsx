...
import { fetchInvoices } from '../api/queries';
...
function statusVariant(s?: string): 'default' | 'secondary' | 'destructive' | 'outline' {
  ...
  if (
    ...
    return 'secondary';
  if (s.includes('PAID')) return 'default';
...
export function InvoicesPageClient() {
  ...
  const handleSearch = useCallback(() => {
    setFilters({
      ...
      contractId: contractIdInput.trim() || undefined,
      quoteId: quoteIdInput.trim() || undefined,
    });
    setPagination((p) => ({ ...p, pageIndex: 0 }));
  }, [contractIdInput, quoteIdInput, statusInput]);
  ...
  const columns = useMemo<ColumnDef<InvoiceRow>[]>(
    () => [
      {
        id: 'invoiceNumber',
        header: t('common.reference') || 'Invoice',
        cell: ({ row }) => (
          <span className="font-medium">{row.original.invoiceNumber || row.original.id}</span>
        ),
      },
      {
        id: 'status',
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <span>{inv.amount != null ? `${inv.amount} ${inv.currency ?? ''}` : '—'}</span>;
        },
      },
      {
        id: 'contractId',
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t('dashboard.invoices.title') || 'Invoices'}
        subtitle={t('dashboard.invoices.subtitle') || 'Browse invoices from the MPE Web app.'}
        icon={<Receipt className="h-8 w-8 text-[#fe4438]" />}
      />
      ...
      <FilterCard title={t('common.filters')} accentBorder>
        ...
        <Input
          ...
          placeholder={t('common.contractId') || 'Contract id'}
      ...
      <DataTableCard<InvoiceRow>
        ...
        isLoading={isLoading}
        emptyMessage={t('dashboard.invoices.empty') || 'No invoices found.'}
