...
import { Loader2, Send, MessageSquare, Mail, Paperclip, FileIcon, X, Briefcase, AlertCircle } from 'lucide-react';
...
export function QuoteMessagesSheet({ quoteId: initialQuoteId, quote: initialQuote, open, onClose, onSuccess }: QuoteMessagesSheetProps) {
  ...
  // Fetch quote detail when quoteId is selected
  const { data: quoteDetail, isLoading: isLoadingQuoteDetail } = useQuery({
    queryKey: ['agent', 'quotes', selectedQuoteId],
    queryFn: () => fetchQuoteDetail(selectedQuoteId),
    enabled: !!selectedQuoteId && open,
  });

  // Use initial quote if provided, otherwise use fetched quote detail
  const quote = initialQuote || quoteDetail || null;

  // Update selected quote ID when initial quoteId changes
  useEffect(() => {
    if (initialQuoteId) {
  ...
  // Auto-select client when quote is available
  useEffect(() => {
    if (quote && quote.client && !msgRecipientId) {
      setMsgRecipientId(quote.client.id);
    }
  }, [quote, msgRecipientId]);

  const sendMsgMu = useMutation({
    mutationFn: (body: { recipientId: string; content: string; subject?: string; attachments?: MessageAttachment[]; quoteId: string }) =>
      sendMessage({ ...body, quoteId: body.quoteId }),
    onSuccess: () => {
      setMsgContent('');
      setMsgSubject('');
      setAttachments([]);
      queryClient.invalidateQueries({ queryKey: ['agent', 'quotes', selectedQuoteId] });
      onSuccess();
  ...
  const handleSend = () => {
    if (!selectedQuoteId) {
      toast.error(t('dashboard.quotes.selectQuoteFirst') || 'Please select a quote first');
      return;
    }
    if (!msgRecipientId) {
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (attachments.length + fileArray.length > 3) {
        toast.error(t('dashboard.quotes.maxAttachmentsReached') || 'Maximum 3 attachments allowed');
      ...
      const isWord = (t: string) =>
        t === 'application/msword' || t === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
      const isValidType = (t: string) => isImage(t) || isPDF(t) || isWord(t);

      // Validate all files before starting uploads
      for (const file of fileArray) {
        if (file.size > maxSize) {
          toast.error(`${file.name}: ${t('dashboard.quotes.fileTooLarge') || 'File size exceeds 10MB'}`);
          if (fileInputRef.current) fileInputRef.current.value = '';
          return;
        }
      ...
      results.forEach((result, i) => {
        if (result.status === 'fulfilled') {
          ...
          errors.push(fileArray[i].name + ': ' + (result.reason?.message ?? t('dashboard.quotes.uploadFailed')));
        }
      });
      ...
      if (uploadedAttachments.length > 0) {
        ...
        if (uploadedAttachments.length === fileArray.length) {
          toast.success(t('dashboard.quotes.filesUploaded') || 'Files uploaded successfully');
        } else {
          toast.warning(
            `${uploadedAttachments.length} of ${fileArray.length} uploaded. ${errors.join(' ')}`
          );
        }
      }
      if (errors.length > 0 && uploadedAttachments.length === 0) {
        toast.error(errors[0] || t('dashboard.quotes.uploadFailed') || 'Failed to upload files');
      }
  ...
  const isFormValid = useMemo(
    () => selectedQuoteId && msgRecipientId && msgContent.trim(),
    [selectedQuoteId, msgRecipientId, msgContent]
  );
      ...
      <div className="space-y-4 sm:space-y-6 px-0 sm:px-1">
        ...
        <div className="space-y-2 sm:space-y-3">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              ...
              <span className="truncate">{t('dashboard.quotes.selectQuote') || 'Select Quote'}</span>
          ...
          {isLoadingQuotes ? (
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 text-sm text-red-700 p-4 bg-red-50 rounded-lg border border-red-200">
                ...
                <div>
                  <p className="font-medium">Unable to load quotes</p>
                  <p className="text-xs mt-1 opacity-90">{t('dashboard.quotes.quotesLoadError') || 'Please check your connection and try again.'}</p>
                </div>
              </div>
              <Button variant="outline" size="sm" onClick={() => refetchQuotes()} className="w-full h-10">
            ...
            <div className="flex items-start gap-3 text-sm text-amber-700 p-4 bg-amber-50 rounded-lg border border-amber-200">
              ...
              <div>
                ...
                <p className="text-xs mt-1 opacity-90">{t('dashboard.quotes.noQuotesAvailable') || 'Please create a quote first to send messages.'}</p>
              </div>
            </div>
          ) : (
            <>
              <Select value={selectedQuoteId} onValueChange={handleQuoteChange}>
                ...
                >
                  {selectedQuoteId ? (
                    <SelectValue>
                      {(() => {
                        const selectedQuote = quotes.find((q: QuoteRow) => q.id === selectedQuoteId);
                        if (!selectedQuote) return null;

                        return (
                          <div className="flex items-center justify-between w-full pr-2">
                            <div className="flex flex-col gap-1">
                              <span className="font-semibold text-sm text-foreground">{selectedQuote.referenceNumber}</span>
                              <span className="text-xs flex items-center gap-1 text-muted-foreground">
                                <Briefcase className="h-3 w-3 text-primary" />
                                {selectedQuote.client.firstName} {selectedQuote.client.lastName}
                              </span>
                            </div>
                            <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${statusVariant(selectedQuote.status) === 'secondary' ? 'bg-blue-100 text-blue-700' :
                                statusVariant(selectedQuote.status) === 'default' ? 'bg-green-100 text-green-700' :
                                  statusVariant(selectedQuote.status) === 'destructive' ? 'bg-red-100 text-red-700' :
                    ...
                    <SelectValue placeholder={t('dashboard.quotes.chooseQuotePlaceholder') || 'Choose a quote to continue...'} />
                  )}
                </SelectTrigger>
                <SelectContent className="max-h-[300px] bg-popover border-border">
                  {quotes.map((quoteItem: QuoteRow) => {
                    const statusColor =
                      statusVariant(quoteItem.status) === 'secondary' ? 'bg-blue-100 text-blue-700' :
                        statusVariant(quoteItem.status) === 'default' ? 'bg-green-100 text-green-700' :
                          statusVariant(quoteItem.status) === 'destructive' ? 'bg-red-100 text-red-700' :
                            'bg-gray-100 text-gray-700';

                    return (
                      <SelectItem key={quoteItem.id} value={quoteItem.id} className="py-3 cursor-pointer transition-all hover:bg-accent">
                        <div className="flex flex-col gap-2 w-full">
                          <div className="flex items-center justify-between gap-2">
                            <span className="font-semibold text-sm text-foreground">{quoteItem.referenceNumber}</span>
                          ...
                          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                            <Briefcase className="h-3 w-3 text-primary" />
                            <span>{quoteItem.client.firstName} {quoteItem.client.lastName}</span>
                          </div>
                        </div>
                      </SelectItem>
                    );
              ...
              <div className="flex items-start gap-2 p-2.5 sm:p-3 rounded-lg border border-border bg-card">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">
                  {t('dashboard.quotes.quoteHelper') || 'Your message will be sent to the client or assigned agent for the selected quote'}
                </p>
              </div>
            </>
          )}
        ...
        {selectedQuoteId && quote && (
          <div className="space-y-2 sm:space-y-3">
            ...
            <Select value={msgRecipientId} onValueChange={setMsgRecipientId}>
              ...
              <SelectContent className="max-h-[300px] bg-popover border-border">
                <SelectItem value={quote.client.id} className="py-3 cursor-pointer transition-all hover:bg-accent">
                  {quote.client.firstName} {quote.client.lastName} ({t('common.client')})
                </SelectItem>
                {quote.assignedAgent && (
                  <SelectItem value={quote.assignedAgent.id} className="py-3 cursor-pointer transition-all hover:bg-accent">
                    {quote.assignedAgent.firstName} {quote.assignedAgent.lastName} ({t('dashboard.quotes.agent')})
                  </SelectItem>
                )}
              </SelectContent>
            </Select>
        ...
        <div className="space-y-2 sm:space-y-2.5">
          ...
          <div className="flex justify-between items-center gap-2">
            <p className="text-xs text-muted-foreground truncate">{t('dashboard.quotes.messageSubject')}</p>
          ...
          {attachments.length > 0 && (
            <div className="space-y-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
              ...
              {attachments.map((attachment, index) => (
                <div key={index} className="flex items-center gap-3 p-2.5 bg-white rounded-md border border-slate-200 shadow-sm">
          ...
          >
            ...
            {isUploading ? (
              <>
                ...
                <span className="truncate">{t('dashboard.quotes.uploading') || 'Uploading...'}</span>
              </>
            ) : (
              <span className="truncate">
                {attachments.length === 0 ? t('dashboard.quotes.addFiles') : t('dashboard.quotes.addMoreFiles')} ({attachments.length}/3)
              </span>
            )}
          </Button>
          {attachments.length === 0 && (
    ...
    <Sheet open={open} onOpenChange={(isOpen) => { if (!isOpen) handleClose(); }}>
      <SheetContent side="right" className="w-full sm:w-[540px] flex flex-col p-0">
        <SheetHeader className="px-6 pt-6 pb-4 border-b">
          ...
          <SheetDescription>{t('dashboard.quotes.composeMessage') || 'Compose a new message'}</SheetDescription>
        </SheetHeader>
