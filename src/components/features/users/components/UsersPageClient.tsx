...
export function UsersPageClient() {
  ...
  const createMu = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreateOpen(false);
      toast.success(t('dashboard.users.userCreated'));
    },
    onError: (e: Error) => toast.error(e?.message || t('dashboard.users.failedCreate')),
  });

  const updateMu = useMutation({
    mutationFn: ({ id, data: patch }: { id: string; data: Parameters<typeof updateUser>[1] }) =>
      updateUser(id, patch),
    ...
    onError: (_err, _vars, context) => {
      ...
      toast.error(t('dashboard.users.failedUpdate'));
    },
    onSuccess: () => {
      setEditId(null);
  ...
  const deactivateMu = useMutation({
    ...
    onError: (_err, _vars, context) => {
      if (context?.previous) qc.setQueryData(queryKey, context.previous);
      toast.error(t('dashboard.users.failedDeactivate'));
    },
    onSuccess: () => {
      setDeactivateId(null);
      toast.success(t('dashboard.users.userDeactivated'));
    },
    onSettled: () => qc.invalidateQueries({ queryKey: ['admin', 'users'] }),
  });
  ...
  const columns = useMemo<ColumnDef<UserRow>[]>(
    () => [
      ...
      { id: 'email', accessorKey: 'email', header: t('common.email') },
      {
        ...
        cell: ({ row }) => (
          <Badge variant="outline">{t(getRoleTranslationKey(row.original.role))}</Badge>
        ),
      },
      {
        id: 'isActive',
      ...
      },
      {
        ...
        header: t('dashboard.users.quotesAssigned'),
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: t('dashboard.users.created'),
        cell: ({ row }) => (
          <span className="text-muted-foreground text-sm">
    ...
    <div className="space-y-6">
      ...
      {deactivateId && canDeactivateUser && (
        <ConfirmDialog
          ...
          title={t('dashboard.users.deactivateTitle')}
          description={t('dashboard.users.deactivateDescription')}
          confirmLabel={t('dashboard.users.deactivate')}
